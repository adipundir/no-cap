...
contract NOCAPFactChecker {
        ...
        returns (uint256 factId) 
    {
        ...
        // Initialize fact storage reference to avoid stack issues
        Fact storage newFact = facts[factId];
        newFact.id = factId;
        newFact.creator = msg.sender;
        newFact.walrusBlobId = walrusBlobId;
        newFact.creatorStake = msg.value;
        newFact.capVotes = msg.value > 0 ? 1 : 0;
        newFact.noCapVotes = 0;
        newFact.totalCapStake = msg.value;
        newFact.totalNoCapStake = 0;
        newFact.createdAt = block.timestamp;
        newFact.deadline = block.timestamp + VOTING_PERIOD;
        newFact.resolved = false;
        newFact.outcome = false;
        newFact.totalRewards = 0;
    ...
    function _distributeRewards(uint256 factId, bool outcome, uint256 totalPool) internal {
        ...
        for (uint256 i = 0; i < votesLength;) {
            ...
            if (currentVote.vote == outcome && currentVote.stake > 0) {
                withdrawableBalance[currentVote.voter] += (currentVote.stake * totalPool) / winningStake;
            }
    ...
    function getPaginatedFacts(uint256 startIndex, uint256 pageSize) external view returns (Fact[] memory factsArray) {
        ...
        factsArray = new Fact[](resultLength);
        
        for (uint256 i = 0; i < resultLength;) {
            factsArray[i] = facts[startIndex + i];
            unchecked { ++i; }
        }
    }
